//! Stored-config crash-shipping smoke tests.
...
async fn stored_config_shipper_recovers_endpoint_and_posts_to_server() {
    ...
    std::fs::write(&context_file, br#"{"stored":true}"#).expect("write context file");
    ...
    let result = std::thread::spawn(|| panic!("stored config smoke")).join();
...
#[tokio::test]
async fn stored_config_shipper_errors_when_config_is_missing() {
    let spool = TempDir::new().expect("spool dir");
    let pending = spool.path().join("pending");
    let sent = spool.path().join("sent");
    let entry = pending.join("legacy-entry");
    std::fs::create_dir_all(&entry).expect("create legacy entry");

    let error =
        ship_pending_crashes_using_stored_config(spool.path(), SecretString::from("secret-token"))
            .await
            .expect_err("missing stored config is an error");
...
fn config(endpoint: url::Url, spool_dir: PathBuf, context_file: PathBuf) -> PanicHookConfig {
    PanicHookConfig {
        ...
        context: json!({"test": "stored_config_shipper_recovers_endpoint_and_posts_to_server"}),
